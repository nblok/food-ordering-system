...
class OrderCreateHelper:
    ...
    def persist_order(self, command: CreateOrderCommand) -> OrderCreatedEvent:
        ...
        order_created_event = self.order_domain_service.validate_and_initiate_order(
            order, restaurant
        )
    ...
    def check_restaurant(self, command: CreateOrderCommand):
        ...
        optionalRestaurant = self.restaurant_repository.find_restaurant_information(
            restaurant
        )
        if not optionalRestaurant:
            logging.warning(f"could not find restaurant {command.restaurant_id}")
            raise OrderDomainException(
                f"could not find restaurant {command.restaurant_id}"
            )
