...
from order_service.order_domain.application_service.dto.track.track_order_response import (
    TrackOrderResponse,
)
...
class OrderDataMapper:
    def create_order_command_to_restaurant(
        ...
        return (
            ...
            .with_products(
                list(
                    map(lambda item: Product(ProductId(item.product_id)), command.items)
                )
            )
            .build()
        )
    ...
    def create_order_command_to_order(self, command: CreateOrderCommand) -> Order:
        return (
            ...
            .with_restaurant_id(RestaurantId(command.restaurant_id))
            .with_street_address(self.order_address_to_street_address(command.address))
            .with_price(Money(command.price))
            .with_items(self.order_items_to_order_item_entities(command.items))
            .build()
        )

    def order_to_create_order_response(
    ...
    def order_items_to_order_item_entities(
    ...
    def order_address_to_street_address(self, address: OrderAddress) -> StreetAddress:
        return StreetAddress(
            uuid.uuid4(), address.street, address.postal_code, address.city
        )

    def order_to_track_order_response(self, order: Order) -> TrackOrderResponse:
        return TrackOrderResponse.model_validate(
            dict(
